...
public class HttpConnection implements Connection {
    ...
    public static class Request extends Base<Connection.Request> implements Connection.Request {
        ...
        private boolean ignoreContentType = false;
      	...
      	private Request() {
            timeoutMilliseconds = Config.preferenceStore().getInt(Config.CONNECTTIMEOUT);
    ...
    public static class Response extends Base<Connection.Response> implements Connection.Response {
        ...
        static Response execute(Connection.Request req, Response previousResponse) throws IOException {
            ...
            URLConnection conn = createConnection(req);
            conn.connect();
            if(conn.getClass().getName().endsWith("sun.net.www.protocol.https.HttpsURLConnectionImpl")){
            	return executeHttps((HttpsURLConnection) conn,req,previousResponse);
            }else{
            	return executeHttp((HttpURLConnection) conn,req,previousResponse);
            }
        }
        
        static Response executeHttp(HttpURLConnection conn,Connection.Request req, Response previousResponse) throws IOException {
        ...
        static Response executeHttps(HttpsURLConnection conn,Connection.Request req, Response previousResponse) throws IOException {
        	if (req.method() == Connection.Method.POST)
                writePost(req.data(), conn.getOutputStream());          
        ...
        private static URLConnection createConnection(Connection.Request req) throws IOException {
        	HttpsURLConnection.setDefaultHostnameVerifier(ignoreHostnameVerifier);
        	
        	URLConnection conn =  req.url().openConnection();
        	if(conn.getClass().getName().endsWith("sun.net.www.protocol.https.HttpsURLConnectionImpl")){
        		// Prepare SSL Context
        		...
        		((HttpsURLConnection) conn).setRequestMethod(req.method().name());
                ((HttpsURLConnection) conn).setInstanceFollowRedirects(false); // don't rely on native redirection support
        		...
        		((HttpURLConnection) conn).setRequestMethod(req.method().name());
                ((HttpURLConnection) conn).setInstanceFollowRedirects(false); // don't rely on native redirection support
                conn.setConnectTimeout(req.timeout());
                conn.setReadTimeout(req.timeout());
                if (req.method() == Method.POST)
                    conn.setDoOutput(true);
